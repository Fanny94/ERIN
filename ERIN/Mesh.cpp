...
//http://www.braynzarsoft.net/viewtutorial/q16390-22-loading-static-3d-models-obj-format
...
bool Mesh::LoadObjModel(wstring filename, ID3D11Buffer** vertBuff, ID3D11Buffer** indexBuff, vector<int>& subsetIndexStart, vector<int>& subsetMaterialArray, vector<SurfaceMaterial>& material, int& subsetCount, bool isRHCoordSys, bool computeNormals)
{
	HRESULT hr = 0;
							...
							if (checkChar == 'K')
							{
								...
								if (checkChar == 'd')
								{
									...
									if (!alreadyLoaded)
									{
										...
										hr = CreateWICTextureFromFile(graphics->get_gDevice(), fileNamePath.c_str(), NULL, &tempMeshSRV, 0);
										if (SUCCEEDED(hr))
										{
